...
def execute_sparql_query(sensor_name, sensor_location, start_time, end_time):
    ...
    WHERE {{
      ...
      FILTER (?timestamp >= "{start_time}"^^xsd:dateTime && ?timestamp <= "{end_time}"^^xsd:dateTime)
    }}
...
def on_submit():
    ...
    selected_start_time = start_time_entry.get()
    selected_end_time = end_time_entry.get()
    execute_sparql_query(selected_device, selected_location, selected_start_time, selected_end_time)
...
# Start time input
start_time_label = ttk.Label(root, text="Start Time (YYYY-MM-DDTHH:mm:ss):")
start_time_entry = ttk.Entry(root)

# End time input
end_time_label = ttk.Label(root, text="End Time (YYYY-MM-DDTHH:mm:ss):")
...
end_time_label.grid(row=3, column=0, padx=10, pady=5, sticky="w")
